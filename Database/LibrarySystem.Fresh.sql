...
IF DB_ID(N'LibrarySystem') IS NOT NULL
BEGIN
...
(
    ...
    Publication NVARCHAR(256) NOT NULL,
    EntryDate DATETIME NOT NULL CONSTRAINT DF_PublicationMst_EntryDate DEFAULT (GETDATE())
);
GO
...
(
    ...
    Image NVARCHAR(500) NOT NULL CONSTRAINT DF_StudentMst_Image DEFAULT (N'~/img/std.png'),
    EntryDate DATETIME NOT NULL CONSTRAINT DF_StudentMst_EntryDate DEFAULT (GETDATE())
);
GO
...
(
    ...
    SID INT NOT NULL,
    Days INT NOT NULL,
    IssueDate DATETIME NOT NULL CONSTRAINT DF_RentMst_IssueDate DEFAULT (GETDATE()),
    ReturnDate DATETIME NULL,
    Status INT NOT NULL CONSTRAINT DF_RentMst_Status DEFAULT (0)
);
GO
...
(
    ...
    Panalty NUMERIC(18,2) NOT NULL CONSTRAINT DF_PenaltyMst_Panalty DEFAULT (0),
    Detail NVARCHAR(500) NOT NULL CONSTRAINT DF_PenaltyMst_Detail DEFAULT (N''),
    EntryDate DATETIME NOT NULL CONSTRAINT DF_PenaltyMst_EntryDate DEFAULT (GETDATE())
);
GO
...
(
    ...
    Password NVARCHAR(256) NOT NULL,
    EntryDate DATETIME NOT NULL CONSTRAINT DF_AdminMst_EntryDate DEFAULT (GETDATE())
);
GO
...
(
    ...
    AvailableQnt INT NOT NULL,
    RentQnt INT NOT NULL,
    Image NVARCHAR(1000) NOT NULL,
    EntryDate DATETIME NOT NULL CONSTRAINT DF_BookMst_EntryDate DEFAULT (GETDATE())
);
GO

ALTER TABLE dbo.RentMst
ADD CONSTRAINT FK_RentMst_StudentMst FOREIGN KEY (SID) REFERENCES dbo.StudentMst(SID);
GO

ALTER TABLE dbo.PenaltyMst
ADD CONSTRAINT FK_PenaltyMst_StudentMst FOREIGN KEY (SID) REFERENCES dbo.StudentMst(SID);
GO

CREATE UNIQUE INDEX IX_AdminMst_UserName ON dbo.AdminMst(UserName);
GO

CREATE UNIQUE INDEX IX_StudentMst_Email ON dbo.StudentMst(Email);
GO

CREATE INDEX IX_StudentMst_BranchName ON dbo.StudentMst(BranchName);
GO

CREATE INDEX IX_BookMst_Publication ON dbo.BookMst(Publication);
GO

CREATE INDEX IX_BookMst_Branch ON dbo.BookMst(Branch);
GO

CREATE INDEX IX_RentMst_SID_Status ON dbo.RentMst(SID, Status);
GO

CREATE INDEX IX_PenaltyMst_SID_Panalty ON dbo.PenaltyMst(SID, Panalty);
GO

INSERT INTO dbo.AdminMst (Name, UserName, Password, EntryDate)
VALUES (N'Administrator', N'admin', N'admin123', GETDATE());
GO
